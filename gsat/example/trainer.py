...
def run_one_epoch(gsat, data_loader, epoch, phase, dataset_name, seed, use_edge_attr, multi_label,metric_func):
    ...
    for idx, data in enumerate(pbar):
        ...
        desc, _, _, _, _ = log_epoch(epoch, phase, loss_dict, exp_labels, att, data.y.data.cpu(), clf_logits,
                                     dataset_name, seed, multi_label, batch=True,metric_func=metric_func)
        ...
        if idx == loader_len - 1:
            ...
            desc, att_auroc, clf_acc, clf_roc, avg_loss = log_epoch(epoch, phase, all_loss_dict, all_exp_labels, all_att, all_clf_labels, all_clf_logits,
                                                                    dataset_name, seed, multi_label, batch=False,metric_func=metric_func)
        pbar.set_description(desc)
    print (f' clf_acc: {clf_acc:.4f}')
    return att_auroc, None, clf_acc, clf_roc, avg_loss


def log_epoch(epoch, phase, loss_dict, exp_labels, att, clf_labels, clf_logits, dataset_name, seed, multi_label, batch,metric_func):
    desc = f'[Seed {seed}, Epoch: {epoch}]: gsat_{phase}........., ' if batch else f'[Seed {seed}, Epoch: {epoch}]: gsat_{phase} finished, '
    for k, v in loss_dict.items():
        desc += f'{k}: {v:.3f}, '
        
    eval_desc, att_auroc, clf_acc, clf_roc = get_eval_score(exp_labels, att, clf_labels, clf_logits, dataset_name, multi_label, batch,metric_func)
    desc += eval_desc
    return desc, att_auroc, clf_acc, clf_roc, loss_dict['pred']


def get_eval_score(exp_labels, att, clf_labels, clf_logits, dataset_name, multi_label, batch,metric_func):
    clf_preds = get_preds(clf_logits, multi_label)
    clf_acc = 0 if multi_label else (clf_preds == clf_labels).sum().item() / clf_labels.shape[0]
    if batch:
        return f'clf_acc: {clf_acc:.4f}', None, None, None
    ...
    if 'hiv' in dataset_name:
        clf_roc = metric_func(clf_logits,clf_labels)
    if 'drugood' in dataset_name:
        clf_roc = metric_func(clf_logits,clf_labels)
    if 'sst2' in dataset_name:
        clf_roc = metric_func(clf_logits.view(-1,),clf_labels.view(-1,))
    if 'twitter' in dataset_name:
        clf_roc = metric_func(clf_logits,clf_labels) 
    if 'cmnist' in dataset_name:
        clf_roc = metric_func(clf_logits,clf_labels)
    att_auroc = roc_auc_score(exp_labels, att) if np.unique(exp_labels).shape[0] > 1 else 0

    desc = f'clf_acc: {clf_acc:.4f}, clf_roc: {clf_roc:.4f}, att_roc: {att_auroc:.3f}'
...
def update_best_epoch_res(gsat, train_res, valid_res, test_res, metric_dict, dataset_name, epoch, model_dir):
    ...
    if 'ogb' in dataset_name or 'drugood' in dataset_name:
        main_metric_idx = 3  # roc
    else: 
        main_metric_idx = 2  # acc  # for sst2, twitter, mnist
