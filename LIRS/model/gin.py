...
class GIN(BaseModel):
    def __init__(self, nfeat, nhid, nclass, nlayers=2, dropout=0.0,edge_dim=-1,jk='last',node_cls=False,pooling='sum',
        ...
        self.mol_encoder = args.get('mol_encoder',False)
        
        self.is_ogbg = self.mol_encoder
        print (colored(f'use MolEncoder: {self.mol_encoder}','yellow'))
        
        self.jk = jk
        self.node_cls = node_cls
        self.edge_dim = edge_dim
        print ('edge_sim', edge_dim)
        self.layers = nn.ModuleList([])
        if self.mol_encoder and edge_dim>0:
            self.node_encoder = AtomEncoder(emb_dim=nhid)
            self.edge_encoder = BondEncoder(emb_dim=nhid)
            print ('use GOOD encoder')
    ...
    def forward(self, x, edge_index, edge_attr = None,edge_weight=None,batch=None,return_both_rep=False):
        ...
        cond = self.mol_encoder
        if cond and self.edge_dim>0:
