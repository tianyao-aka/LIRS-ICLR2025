...
class DualBranchContrast(torch.nn.Module):
    def __init__(self, loss: Loss, mode: str, intraview_negs: bool = False,biased=False,device=0,ignore=False, **kwargs):
    ...
    def forward(self, h1=None, h2=None, g1=None, g2=None, batch=None, h3=None, h4=None,
        ...
        # re-adjust the mask
        if self.biased:
            pos_mask_adj = generate_mask(data.num_graphs, data.x.shape[0], data.ptr, data.adj_node_indices, data.node_mask, data.node_indices_len)
            pos_mask_adj = pos_mask_adj.to(self.device)
            
            pos_mask1 = pos_mask1*pos_mask_adj
            pos_mask2 = pos_mask2*pos_mask_adj
            if not self.ignore:
                neg_mask1 = 1. - pos_mask1
                neg_mask2 = 1. - pos_mask2
