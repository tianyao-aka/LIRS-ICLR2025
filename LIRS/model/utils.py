...
def save_model_params_to_file(model_params, file_path, file_name):
    """
    Save PyTorch model parameters (state_dict) to a specified directory and file name.
    
    Parameters:
    - model_params (dict): The state_dict of the model to be saved.
    - file_path (str): The directory where the file will be saved.
    - file_name (str): The name of the file (without extension).
    
    Output:
    - None: The function saves the model parameters to a .pt file at the specified location.
    """
    # Create the directory if it does not exist
    if not os.path.exists(file_path):
        os.makedirs(file_path)
    ...
    torch.save(model_params, full_file_path)
...
def train_svm_with_calibration(X, y, X_test=None, y_test=None, eval_method='accuracy'):
    ...
    return torch.tensor(logits), train_score-1.2*test_score
