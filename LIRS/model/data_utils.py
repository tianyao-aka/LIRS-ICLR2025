...
class CustomDataset(Dataset):
    def __init__(self, data_list, dataset_name='motif', node_indices=None,node_mask=None, transform=None, pre_transform=None):
    ...
    def get(self, idx):
        ...
        if 'motif' in self.dset_name.lower():
            if 'sp' in self.dset_name.lower():
                indices, boolean_tensor = get_indices_and_boolean_tensor(data.node_label)
            else:
                indices, boolean_tensor = get_indices_and_boolean_tensor(data.node_gt)  # good motif
...
class DatasetWithSpuRep(Dataset):
    def __init__(self, data_list, dataset_name='spmotif', spu_rep=None,cluster_id=None,binary_cluster_id=None,binary_cluster_count = None,svm_logits = None,intra_cluster_pred_logits=None, intra_cluster_labels=None,sample_weights=None, transform=None, pre_transform=None,model_weights=None):
    ...
    def get(self, idx):
        ...
        if self.spu_rep is not None:
            spu_rep = self.spu_rep[idx].view(1,-1)
        ...
        if self.svm_logits is not None:
            data.svm_logits = torch.tensor(self.svm_logits[idx]).view(1,-1)
