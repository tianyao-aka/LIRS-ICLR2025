...
def aggregate_scores(df):
    ...
    groupby_cols = [col for col in df.columns if col not in ['seed', 'val_score1', 'test_score1','val_score2', 'test_score2']]
    
    # Perform the groupby and aggregation
    grouped_df = df.groupby(groupby_cols).agg(
        val_score_mean=('val_score1', 'mean'),
        val_score_std=('val_score1', 'std'),
        test_score_mean=('test_score1', 'mean'),
        test_score_std=('test_score1', 'std'),
        val_score2_mean=('val_score2', 'mean'),
        val_score2_std=('val_score2', 'std'),
        test_score2_mean=('test_score2', 'mean'),
        test_score2_std=('test_score2', 'std'),
        num_seed=('val_score1', 'count')
    ).reset_index()
            ...
            for _, sub_subdirs, sub_files in os.walk(subdir_path):
                for sub_subdir in sub_subdirs:
                    ...
                    if os.path.exists(npy_file_path):
                        ...
                        val_score1, test_score1,val_score2,test_score2 = val_test_metric
                        ...
                        config['val_score1'] = val_score1
                        config['test_score1'] = test_score1
                        config['val_score2'] = val_score2
                        config['test_score2'] = test_score2
                        # print (config)
...
dset = ["SPMotif-0.400","SPMotif-0.599","SPMotif-0.900"]
