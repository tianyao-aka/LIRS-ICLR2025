...
parser.add_argument('--bias',type=str,default="0.40001")
parser.add_argument('--size',type=float,default=1.0)
parser.add_argument('--nb_num',type=int,default=2)
parser.add_argument('--more_edges',type=int,default=0)
args = parser.parse_args()
global_b = args.bias
num_edges = args.more_edges
num_train_perc = 3000
num_val_perc = 1000
num_test_perc = 1000
nb_num = args.nb_num
...
def get_house(basis_type, nb_shapes=80, width_basis=8, feature_generator=None, m=3, draw=True):
    ...
    G, role_id, _ = synthetic_structsim.build_graph(width_basis,
                                                    ...
                                                    list_shapes,
                                                    start=0,
                                                    rdm_basis_plugins=True,add_random_edges=num_edges)
...
for _ in tqdm(range(num_train_perc)):
    ...
    if base_num == 2:
        base = 'ladder'
        width_basis = np.random.choice(range(int(8*size_ratio), int(size_ratio*12)))
    if base_num == 3:
        base = 'wheel'
        width_basis = np.random.choice(range(int(15*size_ratio), int(20*size_ratio)))

    G, role_id, name = get_cycle(basis_type=base,
                                 nb_shapes= nb_num,
                                 width_basis=width_basis,
                                 feature_generator=None,
                                 m=3,
...
for _ in tqdm(range(num_train_perc)):
    ...
    G, role_id, name = get_house(basis_type=base,
                                 nb_shapes= nb_num,
                                 width_basis=width_basis,
                                 feature_generator=None,
                                 m=3,
...
for _ in tqdm(range(num_train_perc)):
    ...
    G, role_id, name = get_crane(basis_type=base,
                                 nb_shapes= nb_num,
                                 width_basis=width_basis,
                                 feature_generator=None,
                                 m=3,
...
np.save(f'../data/SPMotif-{global_b}/raw/train.npy',
        (edge_index_list, label_list, ground_truth_list, role_id_list, pos_list,env_list))
...
print(np.mean(n_mean), np.mean(e_mean))
print(len(ground_truth_list))
np.save(f'../data/SPMotif-{global_b}/raw/test.npy',
        (edge_index_list, label_list, ground_truth_list, role_id_list, pos_list,env_list))
