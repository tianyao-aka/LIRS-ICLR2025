...
if __name__ == '__main__':
    ...
    parser.add_argument('--explain_res_layer',type=int,default=4)
    ...
    args = vars(args)
    if 'spmotif' not in args['dataset'].lower():
        ...
        node_indices_path = f"explain_res/{name}/GIN_layer_{args['explain_res_layer']}/res_dict_top{K}.pt"
    ...
    if not args['biased']:
        embeddingWritingPath = f"experiment_results/SSL_embedding/{args['dataset']}/biased_infomax_{args['biased']}/hidden_dims_{args['hidden_dims']}_num_layers_{args['num_layers']}/"
    else:
        
        if 'spmotif' in args['dataset'].lower():
            embeddingWritingPath = f"experiment_results/SSL_embedding/{args['dataset']}/biased_infomax_{args['biased']}_ignore_{args['ignore']}/hidden_dims_{args['hidden_dims']}_num_layers_{args['num_layers']}/"
        else:
            embeddingWritingPath = f"experiment_results/SSL_embedding/{args['dataset']}/biased_infomax_{args['biased']}_ignore_{args['ignore']}/hidden_dims_{args['hidden_dims']}_num_layers_{args['num_layers']}/expLayer_{args['explain_res_layer']}_expTopK_{args['explain_res_topK']}_expThres_{args['explain_res_thres_index']}/"
    ...
    if args['dataset'].lower().startswith('drugood'):
        ...
        custom_train_dataset = CustomDataset(train_data_list,dataset_name='drugood',node_indices=node_indices,node_mask=node_masks)
        ...
        train_loader = DataLoader(custom_train_dataset,batch_size=args["batch_size"],num_workers=num_workers,shuffle=True)
        # tr_loader = DataLoader(custom_train_dataset,batch_size=3,num_workers=num_workers,shuffle=False)
        # for t in tr_loader:
        #     print (t.node_indices)
        #     print (t.node_mask)
        # valid_loader = DataLoader(val_dataset,batch_size=args["batch_size"],shuffle=False,num_workers=num_workers)
        # test_loader = DataLoader(test_dataset,batch_size=args["batch_size"],shuffle=False,num_workers=num_workers)
        input_dim=4
