...
if "motif" in args['dataset'].lower():
    ...
    if args['raw_labels']:
        print (colored('Use raw labels for spurious logits generation','blue'))
        Y = torch.tensor([train_dataset[i].y.item() for i in range(len(train_dataset))])
        cluster_process = ClusterProcessor(spu_emb,Y,num_classes=args['nclass'], num_clusters = args['nclass'],gamma=args['gamma'])
        intra_cluster_logits,intra_cluster_labels,sample_weights = cluster_process.process_no_cluster()   #! no clustering is performed
...
if "goodhiv" in args['dataset'].lower():
    ...
    if args['raw_labels']:
        print (colored('Use raw labels for spurious logits generation','blue'))
        Y = torch.tensor([train_dataset[i].y.item() for i in range(len(train_dataset))])
        cluster_process = ClusterProcessor(spu_emb,Y,num_classes=2, num_clusters = 2,gamma=args['gamma'])
        intra_cluster_logits,intra_cluster_labels,sample_weights = cluster_process.process_no_cluster()   #! no clustering is performed
...
if "cmnist" in args['dataset'].lower():
    ...
    if args['raw_labels']:
        print (colored('Use raw labels for spurious logits generation','blue'))
        Y = torch.tensor([train_dataset[i].y.item() for i in range(len(train_dataset))])
        cluster_process = ClusterProcessor(spu_emb,Y,num_classes=10, num_clusters = args['nclass'],gamma=args['gamma'])
        intra_cluster_logits,intra_cluster_labels,sample_weights = cluster_process.process_no_cluster()   #! no clustering is performed
...
# res = sorted(res,key = lambda x:x[0],reverse=True)
    val_res = [i[0] for i in res]
    test_res = [i[1] for i in res]
    test1 = select_test_acc(val_res,test_res,k=3)
    test2 = select_test_acc(val_res,test_res,k=5)
    test3 = select_test_acc(val_res,test_res,k=2)
    val_score,test_score = res[0]
    res = np.array([test1,test2,test3])
elif "motif" in args['dataset'] and domain=='size':
    res = sorted(res,key = lambda x:x[0],reverse=True)
    val_score,test_score = res[0]
    res = np.array([val_score,test_score,test_score])
